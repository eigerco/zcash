...
CTransaction::CTransaction(const CMutableTransaction &tx) : nVersion(tx.nVersion), fOverwintered(tx.fOverwintered), nVersionGroupId(tx.nVersionGroupId), nExpiryHeight(tx.nExpiryHeight),
                                                            ...
                                                            vin(tx.vin), vout(tx.vout), nLockTime(tx.nLockTime),
                                                            nZsfDepositAmount(tx.nZsfDepositAmount),
...
CTransaction& CTransaction::operator=(const CTransaction &tx) {
    ...
    *const_cast<CAmount*>(&nZsfDepositAmount) = tx.nZsfDepositAmount;
    saplingBundle = tx.saplingBundle;
    orchardBundle = tx.orchardBundle;
...
{
    CAmount nValueOut{nZsfDepositAmount};
    ...
    if (!fOverwintered) {
        ...
        if (nVersion >= ZIP225_MIN_TX_VERSION) {
            str += strprintf(", nConsensusBranchId=%08x, valueBalanceOrchard=%u, vOrchardAction.size=%u",
                ...
                orchardBundle.GetNumActions());
            if (nVersion >= ZIP233_ZSF_MIN_TX_VERSION) {
                str += strprintf(", nZsfDepositAmount=%u", nZsfDepositAmount);
            }
        }
        str += ")\n";
    } else if (nVersion >= 3) {
