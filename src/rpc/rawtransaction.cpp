...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    if (tx.nVersion >= ZFUTURE_TX_VERSION) {
        entry.pushKV("zsfDeposit", ValueFromAmount(tx.nZsfDepositAmount));
    }
}
...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zsfDeposit\" : x.xxx            (numeric) ZSF deposit amount in " + CURRENCY_UNIT + "\n"
            "}\n"
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "5. zsf-deposit           (numeric, optional, default=0) The amount in " + CURRENCY_UNIT + " to deposit in the ZSF.\n"
            "\nResult:\n"
